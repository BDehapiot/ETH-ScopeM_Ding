...
def get_iips(msk, arr, ddts):
    ...
    global dists, sorts, vals, sort
    
    # Nested function(s) ------------------------------------------------------
    
    def get_iip(dist, sort, vals):
    ...
    for sort in sorts:
        vals = vals[:, sort]
    #     sort = np.tile(sort, (vals.shape[0], 1))
    #     vals = np.take_along_axis(vals, sort, axis=1)
    #     # vals = [val[sort] for val in vals]

    # iips = Parallel(n_jobs=-1)(
    #     delayed(get_iip)(dist, sort, vals)
    #     for (dist, sort) in zip(dists, sorts)
    #     )
