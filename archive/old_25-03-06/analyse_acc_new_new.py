...
exp = np.random.randint(len(acc_data))
clst = np.random.randint(len(acc_data[exp]))
...
grd_tvals_cat = acc_data[exp][clst]["grd_tvals_cat"]
...
for data in acc_data:
    ...
    for tp in range(1, len(tps)):
        tmp_grd_cat.append(np.nanmean(np.concatenate(
            [grd_tvals_cat_acc[tp - 1] for dat in data]).T, axis=1))
...
for tp in range(1, len(tps)):
    ax4.plot(grd_tvals_cat_acc_avg[tp - 1], color=cmap(tp - 1))
